...
서울의 일별 기온을 통해 계절에 따른 흐름과 연도별 차이, 단기간의 큰 변화를 확인하고자 한다. 일상에서 이해하기 쉬운 기온 데이터를 이용해 시계열 분석을 연습한다.
...
- 7일 이동평균: 최근 7일의 평균으로 단기 변동을 완화한다. 완전한 7일이 없는 시작 구간은 계산하지 않는다.
- 연도별 월평균: 같은 월끼리 비교해 계절에 따른 차이를 고려한다. 월별 유효 관측 수도 함께 확인한다.
- 전일 대비 차이: 연속된 날짜의 일평균기온 차이를 ℃로 계산한다. 날짜가 빠진 구간을 하루 변화로 처리하지 않는다.

## 5. 시각화

분석 코드가 아래 그래프 3개를 `images` 폴더에 저장하며, 상대 경로로 리포트에 연결했다.
...
### 5.4 집계 단위를 주별로 바꾼 반례 확인

![주 단위로 다시 집계한 서울 평균기온 변화](images/04_weekly_temperature_change.png)

일 단위 분석에서 나타난 결과가 집계 단위를 바꿔도 유지되는지 확인하기 위해 일평균기온을 월요일부터 일요일까지의 주평균으로 다시 계산했다. 공정한 비교를 위해 7일이 모두 있는 104주만 사용하고, 전주 대비 주평균 차이에 같은 IQR 방식을 적용했다.
...
- 해석 및 반례: 일 단위 결과만 보면 큰 변화가 거의 하락 방향에 집중된 것처럼 보인다. 그러나 주 단위에서는 상승과 하락의 비율이 3대 2로 바뀌었다. 따라서 “큰 기온 변화는 주로 하락이다”라는 해석은 이 데이터의 일 단위 집계에 한정해야 하며, 집계 단위가 달라져도 유지되는 일반적인 결론으로 볼 수 없다.
- 활용: 분석 결론을 제시할 때 집계 단위를 함께 밝히고, 중요한 판단은 일·주·월 단위 결과를 비교한 뒤 내려야 한다.
...
- 일 단위에서는 큰 변화가 하락 쪽에 집중됐지만 주 단위에서는 같은 방향성이 유지되지 않았다. 분석 결과가 집계 단위에 민감하므로 해석할 때 분석 단위를 반드시 함께 제시해야 한다.
...
|---|---|---|
| 분석 질문과 프로젝트 구조 초안 | 과제 요구사항을 정리하기 위해 | 제공된 과제와 대조해 질문 3개, 분석 기법 2개 이상, 그래프 3개 계획을 확인함 |
...
| 이동평균·월평균·전일 차이 계산과 시각화 | 반복 계산을 줄이고 분석 방법의 대안을 탐색하기 위해 | 스크립트를 다시 실행해 같은 수치와 PNG 3개가 재생성되는지 확인하고 그래프의 축·범례·한글 표시를 직접 확인함 |
| 인사이트와 문장 초안 | 관찰과 해석을 구분해 정리하기 위해 | 모든 관찰 수치를 코드 출력과 대조하고, 데이터만으로 확인할 수 없는 원인은 가설로 표현함 |
| 집계 단위 반례 분석 | 일 단위 결론이 다른 집계에서도 유지되는지 검증하기 위해 | 완전한 7일로 구성된 104주를 다시 계산하고, 주 단위 IQR 결과와 일 단위 결과를 수치로 비교함 |
